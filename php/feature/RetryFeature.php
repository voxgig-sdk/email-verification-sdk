...
// EmailVerification SDK retry feature
...
class EmailVerificationRetryFeature extends EmailVerificationBaseFeature
{
    ...
    public function init(EmailVerificationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (EmailVerificationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(EmailVerificationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(EmailVerificationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
