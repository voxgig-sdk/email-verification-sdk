...
// EmailVerification SDK feature test
//
...
class FtCtrl extends EmailVerificationControl
{
...
class FtClient
{
    ...
    private EmailVerificationUtility $utility;

    public function __construct(EmailVerificationUtility $utility, array $options)
    {
    ...
    public function get_utility(): EmailVerificationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (EmailVerificationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public EmailVerificationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (EmailVerificationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = EmailVerificationConfig::make_feature($name);
    ...
    private static function buildUrl(EmailVerificationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof EmailVerificationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new EmailVerificationResponse($fetched);
        }
    ...
    private function fail(EmailVerificationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (EmailVerificationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
