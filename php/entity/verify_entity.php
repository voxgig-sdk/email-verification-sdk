...
// EmailVerification SDK Verify entity
...
class VerifyEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new VerifyEntity($this->_client, $opts);
    }

    /**
     * @param Verify|array $args Verify data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = EmailVerificationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Verify|array The current Verify data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Verify fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = EmailVerificationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Verify fields).
     */
    ...
    /**
     * Load a single Verify.
     *
     * @param VerifyLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed VerifyLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Verify|array The loaded Verify as an assoc-array at the
     *   SDK boundary; throws EmailVerificationError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = EmailVerificationHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
