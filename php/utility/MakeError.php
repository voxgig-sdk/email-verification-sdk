...
class EmailVerificationMakeError
{
    public static function call(?EmailVerificationContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new EmailVerificationContext([], null);
        }
        $op = $ctx->op ?? new EmailVerificationOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new EmailVerificationResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof EmailVerificationError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new EmailVerificationError('', $msg, $ctx);
