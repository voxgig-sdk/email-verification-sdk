...
module EmailVerificationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = EmailVerificationResponse.new({})
    ...
    if fetch_err
      ...
      response = EmailVerificationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = EmailVerificationResponse.new(fetched)
    else
