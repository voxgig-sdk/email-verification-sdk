...
class VerifyEntity
  ...
  def make
    opts = @_entopts.dup
    VerifyEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = EmailVerificationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Verify, Hash] the current Verify data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = EmailVerificationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Verify fields)
  def match_get
  ...
  #
  # @param reqmatch [VerifyLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Verify.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Verify, Hash] the loaded Verify; raises EmailVerificationError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = EmailVerificationHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
