// Command EmailVerification-cli is a CLI + REPL for the EmailVerification SDK,
...
const prompt = "email-verification"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "verify"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      EMAIL_VERIFICATION_APIKEY (api key), EMAIL_VERIFICATION_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: EMAIL_VERIFICATION_APIKEY carries the API key and
	// EMAIL_VERIFICATION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("EMAIL_VERIFICATION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("EMAIL_VERIFICATION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewEmailVerificationSDK(opts)
