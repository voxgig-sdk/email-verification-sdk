# email-verification-cli

boru-driven command-line client **and** interactive REPL for the EmailVerification
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/email-verification-cli)
make build

# 2. See usage (words, entities, env vars)
./email-verification-cli --help

# 3. Provide credentials once, via the environment
export EMAIL_VERIFICATION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./email-verification-cli load 1 verify            # {id:1} shorthand
./email-verification-cli load '{id:1}' verify       # explicit match map

# 5. Override the API base URL for a single call
EMAIL_VERIFICATION_BASE=https://api.example.com ./email-verification-cli load 1 verify

# 6. No arguments -> interactive REPL
./email-verification-cli
email-verification> load 1 verify
email-verification> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/email-verification-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/email-verification-cli load 1 verify
   ```
...
```sh
./email-verification-cli load 1 verify          # scalar shorthand for {id:1}
./email-verification-cli load '{id:1}' verify     # explicit match map
```
...
./email-verification-cli load 1 verify
```
...
Run with no arguments to open a REPL (prompt `email-verification>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `verify`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/email-verification-cli`. |
...
(`./email-verification-cli <expr>`) and interactively (the REPL), and expressions compose the
