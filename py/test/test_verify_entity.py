...
class TestVerifyEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Verify(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _verify_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "verify." + _op, "live" if _live else "unit")
            if _skip:
        ...
        verify_ref01_data = None
        if len(verify_ref01_data_raw) > 0:
            verify_ref01_data = helpers.to_map(verify_ref01_data_raw[0][1])

        # LOAD
        verify_ref01_ent = client.Verify(None)
        verify_ref01_match_dt0 = {}
        verify_ref01_data_dt0_loaded = verify_ref01_ent.load(verify_ref01_match_dt0, None)
        assert verify_ref01_data_dt0_loaded is not None



def _verify_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/verify/VerifyTestData.json")
