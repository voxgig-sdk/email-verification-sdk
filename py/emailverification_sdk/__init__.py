...
from emailverification_sdk.utility import register

# Load features
from emailverification_sdk.feature.base_feature import EmailVerificationBaseFeature
from emailverification_sdk.features import _has_feature, _make_feature
...
class EmailVerificationSDK:
    ...
    def __init__(self, options=None):
        ...
        utility = EmailVerificationUtility()
        self._utility = utility

        from emailverification_sdk.config import shared_config
    ...
    def get_utility(self):
        return EmailVerificationUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("EmailVerificationSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def Verify(self, data=None) -> "VerifyEntity":
        """Entity factory: client.Verify().list() / client.Verify().load({"id": ...})."""
        from emailverification_sdk.entity.verify_entity import VerifyEntity
        return VerifyEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "EmailVerificationSDK":
        if sdkopts is None:
            sdkopts = {}
