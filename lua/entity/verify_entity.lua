...
local VerifyEntity = {}
VerifyEntity.__index = VerifyEntity


function VerifyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, VerifyEntity)
  self._name = "verify"
  self._client = client
...
function VerifyEntity:get_name()
  return self._name
end


function VerifyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return VerifyEntity.new(self._client, opts)
end
...
function VerifyEntity:mark_deleted()
  self._deleted = true
end


function VerifyEntity:deleted()
  return true == self._deleted
end


function VerifyEntity:data_set(args)
  if args ~= nil then
...
function VerifyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function VerifyEntity:match_set(args)
  if args ~= nil then
...
function VerifyEntity:match_get()
...
function VerifyEntity:stream(action, args, callopts)
...
---@param reqmatch VerifyLoadMatch
---@param ctrl? table
---@return Verify
---@return string? err
function VerifyEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function VerifyEntity:_run_op(ctx, post_done)
  local utility = self._utility
