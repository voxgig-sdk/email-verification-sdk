# EmailVerification Lua SDK Reference

Complete API reference for the EmailVerification Lua SDK.
...
```lua
local verify = client:Verify(nil)
```
...
```lua
local result, err = client:Verify():load({ email = "email", key = "key" })
```
