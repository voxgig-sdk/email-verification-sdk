# EmailVerification TypeScript SDK Reference

Complete API reference for the EmailVerification TypeScript SDK.
...
```ts
new EmailVerificationSDK(options?: object)
```
...
#### `EmailVerificationSDK.test(testopts?, sdkopts?)`
...
**Returns:** `EmailVerificationSDK` instance in test mode.
...
Alias for `EmailVerificationSDK.test()`.
...
```ts
const result = await client.Verify().load({ email: 'email', key: 'key' })
```
...
Return the parent `EmailVerificationSDK` instance.
