...
const __1 = require("..");
const SDK_NAME = 'EmailVerificationSDK';
...
const TEST_SEED = { "entity": { "verify": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+EmailVerificationSDK\b/.test(code) ||
        /\bEmailVerificationSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+EmailVerificationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/EmailVerificationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
