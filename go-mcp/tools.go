...
type Args struct {
	Entity string         `json:"entity" jsonschema:"verify"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.EmailVerificationSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "email-verification_list",
		Description: "List records from EmailVerification. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "email-verification_load",
		Description: "Load a single record from EmailVerification. " +
...
func runOp(client *sdk.EmailVerificationSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.EmailVerificationSDK, name string) (sdk.EmailVerificationEntity, error) {
	switch strings.ToLower(name) {
	case "verify":
		return client.Verify(nil), nil
