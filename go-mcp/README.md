...
[MCP](https://modelcontextprotocol.io) server exposing the EmailVerification SDK as
two agent tools — `email-verification_list` and `email-verification_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/email-verification-mcp)
make build

# 2. Provide credentials via the environment
export EMAIL_VERIFICATION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user email-verification \
  -- /absolute/path/to/email-verification-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./email-verification-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// email-verification_list: first page of records
{ "entity": "verify" }
{ "entity": "verify", "query": { } }

// email-verification_load: one record by id
{ "entity": "verify", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/email-verification-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user email-verification \
     -- "$PWD"/dist/*/email-verification-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `email-verification_list` and `email-verification_load` tools now appear
   in new sessions. Ask the agent to *"list verify using email-verification"*
   and it calls `email-verification_list` with `{"entity":"verify"}`.
...
```sh
export EMAIL_VERIFICATION_APIKEY=sk_live_xxx            # API key
export EMAIL_VERIFICATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `email-verification_list` tool
...
### Call the `email-verification_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `email-verification_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `email-verification_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `EMAIL_VERIFICATION_APIKEY` | API key sent with every request. |
| `EMAIL_VERIFICATION_BASE` | Optional override of the API base URL. |
...
```sh
./email-verification-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"email-verification_load","arguments":{"entity":"verify","query":{"id":1}}}}'
```
