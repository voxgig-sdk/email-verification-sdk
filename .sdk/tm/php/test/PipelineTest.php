...
// EmailVerification SDK pipeline test
//
...
{
    private static function utility(): EmailVerificationUtility
    {
        return new EmailVerificationUtility();
    }
    ...
    private static function ctx(array $over = []): EmailVerificationContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new EmailVerificationContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new EmailVerificationOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof EmailVerificationError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new EmailVerificationOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = EmailVerificationMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new EmailVerificationOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = EmailVerificationMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new EmailVerificationOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = EmailVerificationMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EmailVerificationSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = EmailVerificationMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        ...
        [, $err] = EmailVerificationMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new EmailVerificationSpec([]),
            'response' => null,
            'result' => new EmailVerificationResult([]),
        ]);
        [, $err] = EmailVerificationMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new EmailVerificationSpec(['step' => 's']),
            'response' => new EmailVerificationResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new EmailVerificationResult([]),
        ]);
        [, $err] = EmailVerificationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new EmailVerificationSpec(['step' => 's']),
            'response' => new EmailVerificationResponse(self::resp(200, ['v' => 1])),
            'result' => new EmailVerificationResult([]),
        ]);
        [, $err] = EmailVerificationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new EmailVerificationSpec(['step' => 's']),
            'response' => new EmailVerificationResponse(self::resp(200, ['v' => 2])),
            'result' => new EmailVerificationResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        EmailVerificationMakeResponse::call($ctx);
    ...
    {
        $preset = new EmailVerificationResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new EmailVerificationSpec([]),
            'response' => new EmailVerificationResponse([]),
            'result' => new EmailVerificationResult([]),
        ...
        [$got, $err] = EmailVerificationMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new EmailVerificationResult([])]);
        [, $err] = EmailVerificationMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new EmailVerificationSpec([]), 'result' => null]);
        [, $err] = EmailVerificationMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EmailVerificationSpec(['step' => 's']),
            'result' => new EmailVerificationResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new EmailVerificationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EmailVerificationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EmailVerificationSpec(['step' => 's']),
            'result' => new EmailVerificationResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new EmailVerificationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EmailVerificationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EmailVerificationResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new EmailVerificationSpec([]),
            'result' => new EmailVerificationResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = EmailVerificationMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EmailVerificationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new EmailVerificationError('boom', 'boom');
        $utility->fetcher = function (EmailVerificationContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EmailVerificationSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = EmailVerificationMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (EmailVerificationContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = EmailVerificationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(EmailVerificationResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        EmailVerificationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (EmailVerificationContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EmailVerificationSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = EmailVerificationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new EmailVerificationResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new EmailVerificationSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = EmailVerificationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = EmailVerificationMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = EmailVerificationMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new EmailVerificationResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, EmailVerificationDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new EmailVerificationResult(['ok' => false])]);
        $this->expectException(EmailVerificationError::class);
        EmailVerificationDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new EmailVerificationResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', EmailVerificationMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        EmailVerificationMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new EmailVerificationBaseFeature();
        $b = new EmailVerificationBaseFeature();
        EmailVerificationFeatureAdd::call($ctx, $a);
        EmailVerificationFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): EmailVerificationBaseFeature
    {
        $f = new EmailVerificationBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        EmailVerificationFeatureAdd::call($ctx, self::named_feature('a'));
        EmailVerificationFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        EmailVerificationFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        EmailVerificationFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        EmailVerificationFeatureAdd::call($ctx, $replace);
        ...
        EmailVerificationFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): EmailVerificationContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new EmailVerificationSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EmailVerificationPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        EmailVerificationPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new EmailVerificationResponse(['headers' => null]),
            'result' => new EmailVerificationResult([]),
        ]);
        EmailVerificationResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        EmailVerificationResultBody::call($ctx);
