...
// EmailVerification SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = EmailVerificationOmni::makeStructRunner(
    ...
    {
        return EmailVerificationOmni::objmodel(EmailVerificationOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return EmailVerificationOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return EmailVerificationOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(EmailVerificationOmni::structfix($now, Struct::undef()))
                        !== json_encode(EmailVerificationOmni::structfix($val, Struct::undef()))) {
