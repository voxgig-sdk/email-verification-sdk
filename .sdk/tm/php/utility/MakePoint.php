...
// EmailVerification SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class EmailVerificationMakePoint
{
    public static function call(EmailVerificationContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof EmailVerificationError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = EmailVerificationHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = EmailVerificationHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
