...
from projectname_sdk import EmailVerificationSDK
from projectname_sdk.core.spec import EmailVerificationSpec
from projectname_sdk.core.result import EmailVerificationResult
from projectname_sdk.core.response import EmailVerificationResponse
from projectname_sdk.core.operation import EmailVerificationOperation
from projectname_sdk.core.error import EmailVerificationError
from projectname_sdk.core import helpers
from projectname_sdk.feature.base_feature import EmailVerificationBaseFeature
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return EmailVerificationError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_exists(self):
        client = EmailVerificationSDK.test(None, None)
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = EmailVerificationResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_init_basic(self):
        init_client = EmailVerificationSDK.test(None, None)
        ...
        class TestInitFeature(EmailVerificationBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(EmailVerificationBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_live(self):
        ...
        live_client = EmailVerificationSDK({
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = EmailVerificationSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_fetch_def_basic(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = EmailVerificationSpec({
            "base": "http://localhost:8080",
            "prefix": "/api",
            "path": "items/{id}",
            "suffix": "",
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = EmailVerificationSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = EmailVerificationSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = EmailVerificationOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = EmailVerificationSDK.test(None, setup_opts)
