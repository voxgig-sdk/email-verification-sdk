...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("EMAILVERIFICATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("EMAILVERIFICATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("EMAILVERIFICATION_TEST_EXPLAIN"); explain != "" {
