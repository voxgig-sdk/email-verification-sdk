...
func TestVerifyEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set EMAILVERIFICATION_TEST_VERIFY_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func verifyBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("EMAILVERIFICATION_TEST_VERIFY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["EMAILVERIFICATION_TEST_VERIFY_ENTID"])
